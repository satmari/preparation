...
def po_stock(request):
    ...
    ISNULL(bss.to_receive_b, 0) AS to_receive_b,
    ISNULL(bss.stockl_b, 0) AS stockl_b,
    ISNULL(bss.in_prod_b, 0) AS in_prod_b,

    ISNULL(cs.stock_c, 0) AS stock_c,
    ISNULL(cr.request_c, 0) AS request_c,
    ISNULL(css.to_receive_c, 0) AS to_receive_c,
    ISNULL(css.stockl_c, 0) AS stockl_c,
    ISNULL(css.in_prod_c, 0) AS in_prod_c

FROM pos
LEFT JOIN prep_locations AS p ON p.id = pos.loc_id_se
...
LEFT JOIN (
    ...
    GROUP BY po_id
) AS css ON css.po_id = pos.id

WHERE 
    pos.closed_po = 'Open'
    AND posum.location_all = 'Senta'
        ...
        if barcode != '0':
            ...
            if not errors:  # Proceed if no errors so far
                try:
                    ...
                    errors.append("Problem saving to BarcodeSEStocks table")
