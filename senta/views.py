...
def senta_dashboard(request):
    # is_senta = request.user.groups.filter(name='senta').exists()
    # return render(request, 'senta/senta_dashboard.html', {'is_senta': is_senta})
    return render(request, 'senta/senta_dashboard.html')
...
def po_stock(request):
    ...
    with connections['default'].cursor() as cursor:
        cursor.execute("""
    		...
    		(SELECT p.location FROM prep_locations as p WHERE p.id = pos.loc_id_se) as location,
    		...
    		(SELECT SUM([barcode_se_stocks].qty)  FROM [barcode_se_stocks] WHERE [barcode_se_stocks].po_id = pos.id AND [barcode_se_stocks].status = 'to_receive') as to_receive_b,
    		(SELECT SUM([barcode_se_stocks].qty)  FROM [barcode_se_stocks] WHERE [barcode_se_stocks].po_id = pos.id AND [barcode_se_stocks].status != 'to_receive') as stockl_b,
    		(SELECT SUM([barcode_se_stocks].qty)*(-1)  FROM [barcode_se_stocks] WHERE [barcode_se_stocks].po_id = pos.id AND [barcode_se_stocks].type = 'in_line') as in_prod_b,
    		...
    		(SELECT SUM([carelabel_se_stocks].qty)  FROM [carelabel_se_stocks] WHERE [carelabel_se_stocks].po_id = pos.id AND [carelabel_se_stocks].status = 'to_receive') as to_receive_c,
    		(SELECT SUM([carelabel_se_stocks].qty)  FROM [carelabel_se_stocks] WHERE [carelabel_se_stocks].po_id = pos.id AND [carelabel_se_stocks].status != 'to_receive') as stockl_c,
    		(SELECT SUM([carelabel_se_stocks].qty)*(-1)  FROM [carelabel_se_stocks] WHERE [carelabel_se_stocks].po_id = pos.id AND [carelabel_se_stocks].type = 'in_line') as in_prod_c

    		FROM pos
            LEFT JOIN [172.27.161.200].[posummary].dbo.pro as posum ON posum.po_new = pos.po_new

    		WHERE pos.closed_po = 'Open' AND posum.location_all = 'Senta'
    		GROUP BY	pos.id,
    					pos.po,
    					pos.po_new,
    					posum.location_all,
    ...
    return render(request, 'senta/po_stock.html', {'data': data})

def functions(request):
    # return HttpResponse("functions view is working!")

    return render(request, 'senta/functions.html')
...
def give_to_the_line(request):
    ...
    with connections['bbstock_db'].cursor() as cursor:
        cursor.execute("""
            ...
            WHERE location_dest='SENTA' and location_type= 'MODULE/LINE' --and SUBSTRING(location, 0,1) = 'Z'
    ...
    if request.method == 'POST':
        ...
        if barcode != '0':
            ...
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT SUM(qty) as barcode_se_stock
                    FROM barcode_se_stocks
                    WHERE ponum = %s AND status != 'to_receive'
                """, [po_num])
                row = cursor.fetchone()
                barcode_se_stock = row[0] or 0

            if barcode_se_stock - qty < 0:
            ...
            if error_msg_b == "":  # Proceed if no errors so far
                try:
                    ...
                    success_msg += "BarcodeSeStocks uspesno snimljen. <br>"
                except Exception as e:
                    # traceback.print_exc()
        ...
        if carelabel != '0':
            ...
            with connection.cursor() as cursor:
                cursor.execute("""
                    SELECT SUM(qty) as carelabel_se_stocks
                    FROM carelabel_se_stocks
                    WHERE ponum = %s AND status != 'to_receive'
                """, [po_num])
                row = cursor.fetchone()
                carelabel_se_stocks = row[0] or 0

            if carelabel_se_stocks - qty < 0:
            ...
            if error_msg_c == "":  # Proceed if no errors so far
                try:
                    ...
                    success_msg += "CarelabelSeStocks uspesno snimljen."
                except Exception as e:
